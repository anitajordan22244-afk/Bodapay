 ...
 *   REACT_APP_API_URL          - Base URL of the Bodapay backend API
...
function resolveApiUrl() {
  ...
  if (!raw || raw.trim() === "") {
    if (IS_DEV_OR_TEST) {
      console.warn(
        `[Bodapay] REACT_APP_API_URL is not set. Using development fallback: ${DEV_FALLBACK_URL}`
      );
      return DEV_FALLBACK_URL;
    }
    throw new Error(
      "[Bodapay] REACT_APP_API_URL is required but not set. " +
  ...
  if (!isValidUrl(raw)) {
    if (IS_DEV_OR_TEST) {
      console.warn(
        `[Bodapay] REACT_APP_API_URL "${raw}" is not a valid http/https URL. ` +
